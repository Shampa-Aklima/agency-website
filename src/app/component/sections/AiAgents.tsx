          ...
          >
            <div className="aspect-video bg-black/50 backdrop-blur-sm rounded-xl sm:rounded-2xl overflow-hidden border border-white/10 shadow-[0_0_50px_rgba(142,124,255,0.2)]">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center cursor-pointer hover:bg-white/20 transition-colors">
            ...
            <div className="absolute -bottom-4 -right-4 sm:-bottom-5 sm:-right-5 bg-gradient-to-br from-accent-pink to-accent-blue px-4 sm:px-6 py-2 sm:py-3 rounded-lg sm:rounded-xl text-white text-sm sm:text-base font-medium">
            ...
            <div className="space-y-4 sm:space-y-6 mb-6 sm:mb-8">
              {agentFeatures.map((feature, index) => (
                <div key={index} className="flex items-start">
                  <div className="mr-3 sm:mr-4 bg-white/10 p-2 rounded-lg">{feature.icon}</div>
                  <div>
                    <h4 className="text-white text-sm sm:text-base font-medium">{feature.title}</h4>
                    <p className="text-white/70 text-xs sm:text-sm mt-0.5">{feature.description}</p>
                  </div>
                </div>
              ))}
            </div>

            <Button size="lg" className="rounded-full text-sm sm:text-base w-full sm:w-auto">
